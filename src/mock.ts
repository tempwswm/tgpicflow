...
export async function getData (): Promise<unknown[]> {
	...
	let patt1 = /^(.*):\/\/([^\/]*)\/show\/([^\/]*)\/(.*)/;
	let arr = window.location.href.match(patt1);
	let url = "/api/"+arr[3]+"/10"
	...
	for(;i<times;i++){
		...
		if(response.status===200){
				...
				if (Cookies.get('rmads')!=1){
					if (Math.random()<0.4){
						let item = {cover:"/ads/stt.png",link:"https://sttlink.cc/auth/register?code=wswm"};
						let item2 = {cover:"/ads/share1.png",link:"./share.html"};
						data.push(item);
						data.push(item2);
						}
					}
