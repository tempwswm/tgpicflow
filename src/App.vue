...
<!--  <div class="menu">
  ...
  <div :class="{ content: true, 'limit-box': isLimit }">
    ...
    <v3-waterfall :key="forUpdate" class="waterfall" :list="list" srcKey="cover" :gap="12" :colWidth="280"
      :distanceToScroll="400" :scrollBodySelector="isLimit ? '.limit-box' : ''"
      ...
      <template v-slot:default="slotProp">
		...
		<div class="list-item">
		    ...
		    <div class="cover-wrapper">
				<a v-if="slotProp.item.link" :href="slotProp.item.link" target="_blank">
					<img :src="slotProp.item.cover" class="cover" />
				</a>
				<img v-else :src="slotProp.item.cover" class="cover" />
		    </div>
...
export default defineComponent({
  ...
  setup () {
    ...
    //   if (list.value.length > 120){
		  // list.value = list.value.slice(newList.length)
		  // } 
    }
    onMounted(fetchList)
    let isLoad = false
